...
### 5.1 Recommended: train added-token rows only
...
### 6.2 Fine-tuned model (no phrase merge)

```bash
python scripts/eval_retrieval_bioasq.py \
  --model-name-or-path checkpoints/all-MiniLM-L6-v2-bioasq-finetuned-added-only \
  --no-phrase-merge \
  --output-json outputs/eval_added_only_no_merge.json
```

### 6.3 Fine-tuned model (with phrase merge)
